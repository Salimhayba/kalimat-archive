...
// نسخة الكاش: غيّر الرقم ده كل ما تحدّث index.html أو أي ملف داخل data/ عشان يوصل التحديث للمستخدمين
const CACHE_VERSION = 'kalimat-archive-v7';
const CORE_ASSETS = [
  './',
  './index.html',
  './manifest.json',
...
self.addEventListener('install', function(event){
  event.waitUntil(
    ...
    fetch('./data/meta.json').then(function(r){ return r.json(); }).then(function(meta){
      var dataAssets = ['./data/meta.json'].concat((meta.years || []).map(function(y){ return './data/' + y + '.json'; }));
